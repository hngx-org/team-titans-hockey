    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            soundViewModel.isSoundOn.observe(viewLifecycleOwner) { isSoundOn ->
                if (isSoundOn) {
                    mediaPlayer?.start()
                    view.findViewById<ImageView>(R.id.soundbtn)?.setImageResource(R.drawable.volume_max)
                    view.findViewById<TextView>(R.id.switchTv)?.setText(R.string.on)
                } else {
                    mediaPlayer?.pause()
                    view.findViewById<ImageView>(R.id.soundbtn)?.setImageResource(R.drawable.volume_off_02)
